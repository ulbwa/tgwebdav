...
// expects (*RateLimitError, ErrMessageNotFound,
// ErrForbidden).
//
...
// Client is a net/http-backed Telegram Bot API client.
//
...
func (c *Client) pace(ctx context.Context, bot *model.Bot) error {
...
func (c *Client) methodURL(bot *model.Bot, method string) string {
...
func (c *Client) do(ctx context.Context, bot *model.Bot, req *http.Request) (json.RawMessage, error) {
...
//   - "not found" style descriptions   → ErrMessageNotFound.
...
func (c *Client) mapError(env apiResponse) error {
	...
	if strings.Contains(desc, "not found") ||
		...
		return fmt.Errorf("%w: %s", ErrMessageNotFound, env.Description)
	}

	if env.ErrorCode == 403 {
		return fmt.Errorf("%w: %s", ErrForbidden, env.Description)
	}
...
func (c *Client) GetMe(ctx context.Context, bot *model.Bot) (string, error) {
...
func (c *Client) GetChat(ctx context.Context, bot *model.Bot, chatID int64) (string, bool, error) {
	form := url.Values{}
...
func (c *Client) SendDocument(ctx context.Context, bot *model.Bot, chatID int64, filename string, data []byte) (model.TGSendResult, error) {
	var body bytes.Buffer
	mw := multipart.NewWriter(&body)

	if err := mw.WriteField("chat_id", strconv.FormatInt(chatID, 10)); err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: write chat_id field: %w", err)
	}
	part, err := mw.CreateFormFile("document", filename)
	if err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: create document part: %w", err)
	}
	if _, err := part.Write(data); err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: write document bytes: %w", err)
	}
	if err := mw.Close(); err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: finalize multipart body: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.methodURL(bot, "sendDocument"), &body)
	if err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: build sendDocument request: %w", err)
	}
	req.Header.Set("Content-Type", mw.FormDataContentType())

	raw, err := c.do(ctx, bot, req)
	if err != nil {
		return model.TGSendResult{}, err
	}
...
func (c *Client) SendByFileID(ctx context.Context, bot *model.Bot, chatID int64, fileID string) (model.TGSendResult, error) {
	form := url.Values{}
	form.Set("chat_id", strconv.FormatInt(chatID, 10))
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.methodURL(bot, "sendDocument"),
		strings.NewReader(form.Encode()))
	if err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: build sendDocument(file_id) request: %w", err)
	}
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
...
func (c *Client) ForwardMessage(ctx context.Context, bot *model.Bot, toChatID, fromChatID, messageID int64) (model.TGSendResult, error) {
	form := url.Values{}
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.methodURL(bot, "forwardMessage"),
		strings.NewReader(form.Encode()))
	if err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: build forwardMessage request: %w", err)
	}
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
...
func (c *Client) DeleteMessage(ctx context.Context, bot *model.Bot, chatID, messageID int64) error {
	form := url.Values{}
	form.Set("chat_id", strconv.FormatInt(chatID, 10))
	form.Set("message_id", strconv.FormatInt(messageID, 10))
...
func (c *Client) DownloadFile(ctx context.Context, bot *model.Bot, fileID string) ([]byte, error) {
	form := url.Values{}
	...
	if fr.FilePath == "" {
		return nil, fmt.Errorf("%w: empty file_path", ErrMessageNotFound)
	}
...
// CDN is mapped to ErrMessageNotFound; other non-200 responses become a
// wrapped error.
func (c *Client) downloadPath(ctx context.Context, bot *model.Bot, filePath string) ([]byte, error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		return nil, fmt.Errorf("%w: file path %q", ErrMessageNotFound, filePath)
	}
...
func decodeSendResult(raw json.RawMessage) (model.TGSendResult, error) {
	var msg documentResult
	if err := json.Unmarshal(raw, &msg); err != nil {
		return model.TGSendResult{}, fmt.Errorf("telegram: decode message result: %w", err)
	}
	res := model.TGSendResult{MessageID: msg.MessageID}
	if msg.Document != nil {
