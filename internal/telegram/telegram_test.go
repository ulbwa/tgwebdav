...
func testBot() *domain.Bot {
	return &domain.Bot{ID: uuid.New(), Username: "test_bot", Token: "TOKEN", Enabled: true}
}
...
func TestSendDocumentRateLimited(t *testing.T) {
	...
	if !errors.As(err, &rl) {
		t.Fatalf("error = %v (%T), want *domain.RateLimitError", err, err)
	}
...
func TestForbiddenMapping(t *testing.T) {
	...
	if !errors.Is(err, domain.ErrTelegramForbidden) {
